...
function summary_of($type, array $d)
{
    ...
    switch ($type) {
        case 'pledge':
            return $pick(['amount']) . ' ' . (isset($d['currency']) ? $d['currency'] : '') . ' · ' . $pick(['frequency', 'project']);
        case 'screening':
            return $pick(['region', 'screening_type']);
        case 'partner':
            return $pick(['organization', 'interest']);
        case 'volunteer':
            return $pick(['city', 'profession']);
        default:
...
if (empty($_SESSION['auth'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password'])) {
        if ($locked) {
            ...
            $db->prepare('INSERT INTO login_fail (ip_hash, created_at) VALUES (?,?)')->execute([$ip, ccf_now()]);
            sleep(1);
...
if (isset($_GET['export'])) {
    ...
    fputcsv($out, ['id', 'received (Douala time)', 'type', 'status', 'name', 'email', 'phone', 'language', 'details']);
    foreach ($st as $r) {
        ...
        fputcsv($out, array_map($safe, [$r['id'], local_time($r['created_at']), $r['type'], $r['status'], $r['name'], $r['email'], $r['phone'], $r['lang'], implode(' | ', $parts)]));
    }
    exit;
}
...
echo '<div class="bar"><h1>CCF submissions</h1><span><a href="?tab=check">System check</a><form method="post" class="inline"><input type="hidden" name="csrf" value="' . h($_SESSION['csrf']) . '"><button name="logout" value="1" class="ghost">Sign out</button></form></span></div>';

if ($tab === 'check') {
    $checks = [];
    $checks[] = ['PHP version', PHP_VERSION, version_compare(PHP_VERSION, '7.3.0', '>=')];
    $checks[] = ['Database driver', ccf_driver(), true];
    $ok = true;
    try {
    ...
    $checks[] = ['Data folder writable', is_writable(CCF_DATA) ? 'yes' : 'no', is_writable(CCF_DATA)];
    $mailTo = trim($cfg['notify_email']);
    $checks[] = ['Email alerts (notify_email set)', $mailTo !== '' ? $mailTo : 'not set - you will not get emails', $mailTo !== ''];
    $checks[] = ['HTTPS', ccf_is_https() ? 'yes' : 'no - use https:// on the live site', ccf_is_https()];
    ...
    if (ccf_driver() === 'sqlite') {
        $url = '../data/' . basename(ccf_sqlite_path());
...
$labels = ['contact' => 'Contact', 'partner' => 'Partnership', 'volunteer' => 'Volunteers', 'pledge' => 'Pledges', 'screening' => 'Screening'];
...
if (!$rows) {
    ...
    foreach ($rows as $r) {
        ...
        echo ' <small>(' . h($r['lang']) . ')</small></td><td><details><summary>' . h(summary_of($r['type'], $d)) . '</summary><dl>';
        foreach ($d as $k => $v) {
...
{
    echo '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>' . h($title) . ' | CCF admin</title><style>
body{font-family:Arial,Helvetica,sans-serif;margin:0;background:#f7f4fa;color:#18223a;line-height:1.5}
main{max-width:1100px;margin:0 auto;padding:24px 16px}
h1{font-size:24px;margin:0;color:#5a2d82}h2{color:#5a2d82}
...
input[type=password]{display:block;width:100%;max-width:340px;margin-top:6px;padding:10px;border:1px solid #cfc3da;border-radius:6px;font-size:16px}
button{background:#5a2d82;color:#fff;border:0;border-radius:6px;padding:10px 16px;font-weight:700;cursor:pointer;font-size:14px}
button.ghost{background:#fff;color:#5a2d82;border:1px solid #cfc3da;padding:6px 10px;margin:2px}
button.danger{color:#b00020}
.chips{display:flex;gap:8px;flex-wrap:wrap;margin:8px 0}
.chip{background:#fff;border:1px solid #cfc3da;border-radius:999px;padding:5px 12px;text-decoration:none;font-size:14px}
.chip.on{background:#5a2d82;color:#fff;border-color:#5a2d82}.chip b{color:#f08b2c}.chip.on b{color:#ffd19b}
table{border-collapse:collapse;width:100%;background:#fff;border:1px solid #e7dfed;margin-top:12px}
th,td{padding:9px 10px;text-align:left;vertical-align:top;border-bottom:1px solid #eee;font-size:14px}
th{background:#efe8f5}tr.new td:first-child{border-left:4px solid #f08b2c}
...
.warn{color:#b00020;font-weight:700}.ok{color:#167d63}pre{background:#fff;border:1px solid #e7dfed;padding:12px;overflow:auto}
@media(max-width:700px){table,thead,tbody,tr,td,th{display:block}th{display:none}td{border:0}tr{border-bottom:2px solid #e7dfed;margin-bottom:8px}}
